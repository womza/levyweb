...
<?php get_header(); ?>
<?php
$hotel_result_count  = Slzexploore::get_option( 'slz-hotel-result-count' );
$hotel_sort_by       = Slzexploore::get_option( 'slz-hotel-list-sort' );
// result-meta
$show_result = false;
$show_sort = false;
$result_class = 'col-md-7';
$sort_class = 'col-md-5';
if( $hotel_result_count ) {
	$show_result = true;
}
else{
...
<div class="hotel-result-main padding-top padding-bottom">
	...
	<div class="container">
		...
		if( is_tax( 'slzexploore_hotel_cat' ) ){
			$atts['cat_slug'] = $queried_object->slug;
			$_GET['accommodation'] = $queried_object->slug;
			$is_archive = true;
		}
		if( is_tax( 'slzexploore_hotel_facility' ) ){
			$atts['facility_slug'] = $queried_object->slug;
			$_GET['facilities'] = $queried_object->slug;
			$is_archive = true;
		}
		if( is_tax( 'slzexploore_hotel_location' ) ){
		...
		<div class="result-body">
			<div class="row">
					...
					<?php
						...
						if( $show_sort ){
							echo '<div class="'. esc_attr( $sort_class ) .'"><div class="result-filter-wrapper">';
							 do_action('slzexploore_core_result_filter', 'hotel');
							$col = 1;
				...
				<div id='page-sidebar' class="sidebar sidebar-widget <?php echo esc_attr( $sidebar_css ); ?> ">
					<?php
						$sidebar_content = Slzexploore::get_option('slz-hotel-archive-sidebar-content');
						if( $sidebar_content ){
							slzexploore_get_sidebar($sidebar_id);
						}
						else{
							echo do_shortcode('[slzexploore_core_accommodation_search_sc]');
						}
					?>
				</div>
				<?php endif;?>
