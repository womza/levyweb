<?php
/**
 * Widget_Accommodation class.
 * 
 * @since 1.0
 */
class Slzexploore_Widget_Accommodation extends WP_Widget {

	public function __construct() {
		$widget_ops = array( 'classname' => 'widget_slz_widget_accommodation', 'description' => esc_html__( "List of accommodation.", 'exploore') );
		parent::__construct( 'slzexploore_accommodation', esc_html_x( 'SLZ: Accommodation', 'Accommodation widget', 'exploore' ), $widget_ops );
	}

	function form($instance) {
		$default = array(
			'title'           => esc_html__( 'Accommodation', 'exploore' ),
			'limit_post'      => '5',
			'cat_slug'        => '',
			'location_slug'   => '',
			'sort_by'         => '',
		...
		$check_box = array(
			...
			'show_price'       => esc_html__( 'Display price accommdation', 'exploore' ),
		);
		$cat               = Slzexploore_Core_Com::get_tax_options2slug( 'slzexploore_hotel_cat', array('empty'       => esc_html__( '--All Categories--', 'exploore' ) ) );
		$location          = Slzexploore_Core_Com::get_tax_options2slug( 'slzexploore_hotel_location', array('empty'   => esc_html__( '--All Location--', 'exploore' ) ) );
		$sort_arr          = Slzexploore_Core::get_params('sort-accomodation');
		...
		<p>
			<label for="<?php echo  esc_attr( $this->get_field_id('featured_filter') ); ?>"><?php esc_html_e( 'Featured Accomodation', 'exploore' );?></label>
		...
		<p>
			<label for="<?php echo  esc_attr( $this->get_field_id('cat_slug') ); ?>"><?php esc_html_e( 'Accommodation Categories', 'exploore' );?></label>
		...
		<p>
			<label for="<?php echo  esc_attr( $this->get_field_id('location_slug') ); ?>"><?php esc_html_e( 'Accomodation Location', 'exploore' );?></label>
