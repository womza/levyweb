 ...
 * @since 1.0
 */
class Slzexploore_Widget_Car extends WP_Widget {

	public function __construct() {
		$widget_ops = array( 'classname' => 'widget_slz_widget_car', 'description' => esc_html__( "A list of Car.", 'exploore') );
		parent::__construct( 'slzexploore_car', esc_html_x( 'SLZ: Car', 'Car widget', 'exploore' ), $widget_ops );
	}

	function form($instance) {
		$default = array(
			'title'           => esc_html__( 'Car', 'exploore' ),
			'limit_post'      => '5',
			'cat_slug'        => '',
			'location_slug'   => '',
			'sort_by'         => '',
		...
		$check_box = array(
			...
			'show_price'       => esc_html__( 'Display price car', 'exploore' ),
		);
		$cat               = Slzexploore_Core_Com::get_tax_options2slug( 'slzexploore_car_cat', array('empty'       => esc_html__( '--All Car Categories--', 'exploore' ) ) );
		$location          = Slzexploore_Core_Com::get_tax_options2slug( 'slzexploore_car_location', array('empty'   => esc_html__( '--All Car Location--', 'exploore' ) ) );
		$sort_arr          = Slzexploore_Core::get_params('sort-car');
		...
		<p>
			<label for="<?php echo  esc_attr( $this->get_field_id('featured_filter') ); ?>"><?php esc_html_e( 'Featured Car', 'exploore' );?></label>
		...
		<p>
			<label for="<?php echo  esc_attr( $this->get_field_id('cat_slug') ); ?>"><?php esc_html_e( 'Car Categories', 'exploore' );?></label>
		...
		<p>
			<label for="<?php echo  esc_attr( $this->get_field_id('location_slug') ); ?>"><?php esc_html_e( 'Car Location', 'exploore' );?></label>
