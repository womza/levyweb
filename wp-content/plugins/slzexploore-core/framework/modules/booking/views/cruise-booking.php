...
if( !empty( $arr_cabin_type ) ){
	foreach( $arr_cabin_type as $cabin_id ){
		if( !empty( $cabin_id ) ){
			...
			$is_price_person[$cabin_id] = get_post_meta( $cabin_id, 'slzexploore_cabin_is_price_person', true );
			$price_infant[$cabin_id] = get_post_meta( $cabin_id, 'slzexploore_cabin_price_infant', true );
		}
	}
}
...
if( !empty($show_ctf) && $show_ctf == 'contact' ){
	...
	<div class="wrapper-btn margin-top70 slz-book-wrapper">
		<a class="btn btn-maincolor btn-info cf7-book" data-toggle="modal" data-target="#booking-form"  data-cabin="<?php echo esc_attr($cabin_id);?>" data-id="<?php echo esc_attr( $cruise_id );?>" data-json='<?php echo $json_data;?>' data-type="<?php echo esc_attr( esc_attr($post_type) );?>"><?php echo esc_html__( 'book now', 'slzexploore-core' )?>
			
		</a>
	</div>
	<?php
}else{
	printf('<div class="wrapper-btn margin-top70 slz-book-cruise">
	...
	<div class="find-widget find-hotel-widget widget new-style">
		...
		<div class="content-widget cruise-booking slz-booking-wrapper">
			...
			<form class="text-input small-margin-top booking-data">
				<div class="text-box-wrapper">
					...
					<?php
						...
						echo ( $this->hidden_field( 'is_price_person', json_encode( $is_price_person ) ) );
					?>
					</div>
				</div>
				<div class="text-box-wrapper">
				...
				<div class="count text-box-wrapper">
					...
					<div class="input-group">
						...
						<?php
							...
							echo ( $this->hidden_field( 'price_infant', json_encode( $price_infant ) ) );
						?>
			...
			<?php else:
				...
				esc_html_e( 'No Cabin Type is available for this cruise. Please book another cruise.', 'slzexploore-core' );
				echo '</div>';
			 endif; ?>
		</div>
	</div>
