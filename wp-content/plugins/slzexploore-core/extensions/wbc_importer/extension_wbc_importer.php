...
if ( !class_exists( 'ReduxFramework_extension_wbc_importer' ) ) {
    ...
    class ReduxFramework_extension_wbc_importer {
            ...
            if ( isset( $_REQUEST['type'] ) && $_REQUEST['type'] == "import-demo-content" && array_key_exists( $_REQUEST['demo_import_id'], $this->wbc_import_files ) ) {
                ...
                if ( file_exists( $demo_data_loc.'/'.$import_parts['content_file'] ) && is_file( $demo_data_loc.'/'.$import_parts['content_file'] ) ) {
                    ...
                    if ( !isset( $import_parts['imported'] ) || true === $reimporting ) {
                        ...
                        echo esc_html__( "Demo Already Imported", 'slzexploore-core' );
                    }
                }
        ...
        function add_importer_section() {
            ...
            $wbc_importer_label = trim( esc_html( apply_filters( 'wbc_importer_label', esc_html__( 'Demo Importer', 'slzexploore-core' ) ) ) );

            $wbc_importer_label = ( !empty( $wbc_importer_label ) ) ? $wbc_importer_label : esc_html__( 'Demo Importer', 'slzexploore-core' );

			$welcom_link = sprintf('<a href="%1$s" title="%2$s">%2$s</a>',
									admin_url( 'admin.php?page='.SLZEXPLOORE_THEME_PREFIX.'_requirement' ),
									esc_html__( 'Requirements', 'slzexploore-core' )
							);
			$this->parent->sections[] = array(
				'id'     => 'wbc_importer_section',
				'title'  => $wbc_importer_label,
				'desc'   => '<p class="description">'. apply_filters( 'wbc_importer_description', esc_html__( 'Works best to import on a new install of WordPress. You should remove all posts, pages, widgets content before import demo data. When progress go too long ( > 15 mins ), please try to refresh page and re-import, or use other tools in documentation', 'slzexploore-core'). '<br/>' . esc_html__( 'Please install required plugins and go to ', 'slzexploore-core' ).$welcom_link.esc_html__( ' to setting your server before click import demo.', 'slzexploore-core' ) ).'</p>',
